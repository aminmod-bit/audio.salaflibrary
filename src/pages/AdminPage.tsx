      ...
      <div style={{maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
          ...
          <div style={{display:'flex',gap:8}}>
            <button onClick={() => exportJSON('speakers.json', scholars)} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>📥 speakers.json</button>
            <button onClick={() => exportJSON('audio.json', lectures)} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>📥 audio.json</button>
            <button onClick={() => exportJSON('series.json', series)} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>📥 series.json</button>
            <button onClick={handleLogout} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>Выйти</button>
          </div>
        </div>
        ...
        <div style={{display:'flex',gap:12,marginBottom:16}}>
          <select value={batchSpeakerId} onChange={e => setBatchSpeakerId(e.target.value)}
            style={{padding:'8px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'#18181c',color:'#fff',fontSize:13}}>
        ...
        <div style={{marginBottom:24}}>
          ...
          <button onClick={() => fileInputRef.current?.click()} disabled={isProcessing}
            style={{padding:'12px 24px',borderRadius:8,background:isProcessing ? '#222226' : 'linear-gradient(135deg,#1b4332,#2d6a4f)',color:'#fff',fontSize:14,fontWeight:600,border:'none',cursor:isProcessing ? 'default' : 'pointer',display:'flex',alignItems:'center',gap:8}}>
            {isProcessing ? '⏳ Обработка...' : '📁 Добавить аудио с компьютера'}
          </button>
          {uploadHint && <div style={{marginTop:8,fontSize:12,color:'#7c5cfc'}}>{uploadHint}</div>}
          <div style={{marginTop:8,fontSize:12,color:'#6b6b73'}}>Поддержка: MP3, M4A, OGG, WAV · Можно выбрать несколько файлов</div>
        </div>
        ...
        <div style={{display:'flex',gap:8,marginBottom:24}}>
          {(['lectures', 'scholars', 'series'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              style={{padding:'8px 16px',borderRadius:100,fontSize:13,fontWeight:500,border:'1px solid rgba(255,255,255,0.06)',cursor:'pointer',
                background: activeTab === tab ? '#7c5cfc' : 'transparent',
                color: activeTab === tab ? '#fff' : '#a0a0a8',
                borderColor: activeTab === tab ? '#7c5cfc' : 'rgba(255,255,255,0.06)'}}>
        ...
        {/* ─── DRAFTS (Batch Upload) ─── */}
        {drafts.length > 0 && (
          <div style={{marginBottom:24,padding:20,background:'#18181c',border:'2px solid #7c5cfc',borderRadius:12}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:16}}>
              <h2 style={{fontSize:18,fontWeight:600}}>Черновики ({drafts.length})</h2>
              <div style={{display:'flex',gap:8}}>
                <button onClick={saveAllDrafts} style={{padding:'8px 16px',borderRadius:8,background:'#7c5cfc',color:'#fff',fontSize:13,fontWeight:600,border:'none',cursor:'pointer'}}>
                  💾 Сохранить все
                </button>
                <button onClick={() => setDrafts([])} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>
                  Очистить
                </button>
              </div>
            </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              {drafts.map((draft, i) => (
                <div key={i} style={{padding:16,background:'#1c1c20',border:'1px solid rgba(255,255,255,0.06)',borderRadius:10}}>
                  <div style={{display:'grid',gridTemplateColumns:'60px 1fr 1fr 1fr 80px',gap:12,alignItems:'center'}}>
                    ...
                    <input type="number" value={draft.lessonNumber} onChange={e => updateDraft(i, { lessonNumber: parseInt(e.target.value) || 0 })}
                      style={{padding:'6px 8px',borderRadius:6,border:'1px solid rgba(255,255,255,0.06)',background:'#222226',color:'#fff',fontSize:13,width:'100%'}} />

                    {/* Title */}
                    <input value={draft.title} onChange={e => updateDraft(i, { title: e.target.value })}
                      style={{padding:'6px 10px',borderRadius:6,border:'1px solid rgba(255,255,255,0.06)',background:'#222226',color:'#fff',fontSize:13}} />
                  ...
                  {/* Info row */}
                  <div style={{display:'flex',gap:16,marginTop:8,fontSize:11,color:'#6b6b73'}}>
            ...
            {editingLecture && (
              <div style={{background:'#18181c',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:20,marginBottom:16}}>
                ...
                <div style={{display:'flex',gap:8,marginTop:12}}>
                  ...
                  <button onClick={() => setEditingLecture(null)} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>Отмена</button>
                </div>
              </div>
            )}
              ...
              {filteredLectures.map(l => (
                <div key={l.id} style={{display:'flex',alignItems:'center',gap:12,padding:12,background:'#18181c',border:'1px solid rgba(255,255,255,0.06)',borderRadius:10}}>
                  {l.coverImage ? (
                    ...
                    <div style={{width:40,height:40,borderRadius:8,background:'#222226',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0}}>{l.icon}</div>
                  )}
                  ...
                  <button onClick={() => setEditingLecture(l)} style={{padding:'6px 12px',borderRadius:6,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:12,cursor:'pointer'}}>Изменить</button>
            ...
            {editingScholar && (
              <div style={{background:'#18181c',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:20,marginBottom:16}}>
                ...
                <textarea placeholder="Описание" value={editingScholar.description || ''} onChange={e => setEditingScholar({...editingScholar, description: e.target.value})}
                  style={{width:'100%',marginTop:12,padding:'8px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'#1c1c20',color:'#fff',fontSize:13,minHeight:80,resize:'vertical'}} />
                <div style={{display:'flex',gap:8,marginTop:12}}>
                  ...
                  <button onClick={() => setEditingScholar(null)} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>Отмена</button>
                </div>
              </div>
            )}
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}}>
              {filteredScholars.map(s => (
                <div key={s.id} style={{padding:16,background:'#18181c',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12}}>
                  ...
                  <div style={{fontSize:11,color:'#6b6b73'}}>Уроков: {lectures.filter(l => l.scholarId === s.id).length}</div>
                  <div style={{display:'flex',gap:8,marginTop:8}}>
                    <button onClick={() => setEditingScholar(s)} style={{padding:'6px 12px',borderRadius:6,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:12,cursor:'pointer'}}>Изменить</button>
            ...
            {editingSeries && (
              <div style={{background:'#18181c',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:20,marginBottom:16}}>
                ...
                <textarea placeholder="Описание" value={editingSeries.description || ''} onChange={e => setEditingSeries({...editingSeries, description: e.target.value})}
                  style={{width:'100%',marginTop:12,padding:'8px 12px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'#1c1c20',color:'#fff',fontSize:13,minHeight:80,resize:'vertical'}} />
                <div style={{display:'flex',gap:8,marginTop:12}}>
                  <button onClick={saveSeriesItem} style={{padding:'8px 16px',borderRadius:8,background:'#7c5cfc',color:'#fff',fontSize:13,fontWeight:600,border:'none',cursor:'pointer'}}>Сохранить</button>
                  <button onClick={() => setEditingSeries(null)} style={{padding:'8px 16px',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:13,cursor:'pointer'}}>Отмена</button>
                </div>
              </div>
            )}
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}}>
              {filteredSeries.map(s => (
                <div key={s.id} style={{padding:16,background:'#18181c',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12}}>
                  ...
                  <div style={{fontSize:12,color:'#6b6b73',marginBottom:4}}>{s.description || 'Без описания'}</div>
                  <div style={{fontSize:11,color:'#6b6b73'}}>Уроков: {lectures.filter(l => l.seriesId === s.id).length}</div>
                  <div style={{display:'flex',gap:8,marginTop:8}}>
                    <button onClick={() => setEditingSeries(s)} style={{padding:'6px 12px',borderRadius:6,border:'1px solid rgba(255,255,255,0.06)',background:'transparent',color:'#a0a0a8',fontSize:12,cursor:'pointer'}}>Изменить</button>
